...
from bitpost_interface import BitpostInterface
...
class AddressUtils:
    ...
    @classmethod
    def base58check(cls, padded):
        hash1 = hashlib.sha256(padded)
        hash2 = hashlib.sha256(hash1.digest())
        padded += hash2.digest()[:4]
        return base58.b58encode(padded).decode("utf-8")

    @classmethod
    def public_key_to_p2pkh(cls, publicKeyHex):
        padded = (b'\x00') + binascii.unhexlify(cls.HASH160(publicKeyHex))
        return cls.base58check(padded)

    @classmethod
    def public_key_to_p2wpkh(cls, publicKeyHex):
        witnessProgram = '0014' + cls.HASH160(publicKeyHex)
        witnessHash = cls.HASH160(witnessProgram)

        padded = (b'\x05') + binascii.unhexlify(witnessHash)
        return cls.base58check(padded)
