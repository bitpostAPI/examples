...
let master: bip32.BIP32Interface = bip32.fromSeed(seed, bitcoin.networks.testnet);
...
let mainAddress = bitcoin.payments.p2wpkh({ pubkey: mainKey.publicKey, network: bitcoin.networks.testnet});
let changeAddress = bitcoin.payments.p2wpkh({ pubkey: changeKey.publicKey, network: bitcoin.networks.testnet });
...
function getUtxosFromAddress(address: string): Array<TxInput>{
  var resp = request('GET', 'https://blockstream.info/testnet/api/address/' + address + '/utxo');
...
function make_transaction(inputs: Array<TxInput>, outputs: Array<TxOutput>, targetFeerate?: number, txSize?: number): bitcoin.Psbt {
  ...
  let tx: bitcoin.Psbt = new bitcoin.Psbt({network: bitcoin.networks.testnet});
  if(targetFeerate){
